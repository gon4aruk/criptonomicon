    ...
    <div class="container">
      ...
      <template v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
            v-for="(tickerItem, idx) in tickers"
            :key="idx"
            @click="selectTicker(tickerItem)"
            class="bg-white overflow-hidden shadow rounded-lg border-purple-800 border-solid cursor-pointer"
            :class="{ 'border-4': tickerItem === selectedTicker }"
          >
            <div class="px-4 py-5 sm:p-6 text-center">
              <dt class="text-sm font-medium text-gray-500 truncate">
                {{ tickerItem.name }} - USDT
              </dt>
            ...
            <button
              @click.stop="handleDelete(tickerItem)"
...
export default {
  ...
  methods: {
    addTicker() {
      ...
      currentTicker.interval = setInterval(async () => {
        const f = await fetch(
    ...
    handleDelete(ticker) {
      clearInterval(this.tickers.find((t) => t.name === ticker.name).interval);
